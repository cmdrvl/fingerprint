...
get_latest_version() {
  payload=$(fetch "https://api.github.com/repos/cmdrvl/fingerprint/releases/latest" 2>/dev/null || true)
...
resolve_install_dir() {
  if [ -n "${FINGERPRINT_INSTALL_DIR:-}" ]; then
    printf '%s' "$FINGERPRINT_INSTALL_DIR"
    return 0
  fi
  ...
  if [ -z "${HOME:-}" ]; then
    fail "HOME is not set; set FINGERPRINT_INSTALL_DIR instead"
  fi
...
make_temp_dir() {
  if command_exists mktemp; then
    tmp=$(mktemp -d 2>/dev/null || mktemp -d -t fingerprint-install)
  else
    tmp="${TMPDIR:-/tmp}/fingerprint-install-$$"
    mkdir -p "$tmp"
  fi
  printf '%s' "$tmp"
}
...
if [ -z "$version" ]; then
  info "No FINGERPRINT_VERSION set; resolving latest release..."
  version=$(get_latest_version)
fi

version=$(normalize_version "$version")
target=$(detect_target)
asset="fingerprint-$version-$target.tar.gz"
base_url="https://github.com/cmdrvl/fingerprint/releases/download/$version"

install_dir=$(resolve_install_dir)
versioned_binary="$install_dir/fingerprint@$version"
active_binary="$install_dir/fingerprint"
...
info "Installing fingerprint $version for $target"
info "Install dir: $install_dir"

download "$base_url/$asset" "$archive_path"

skip_verify=0
if [ "${FINGERPRINT_NO_VERIFY:-}" = "1" ]; then
  skip_verify=1
  warn "FINGERPRINT_NO_VERIFY=1 set; skipping checksum verification"
fi
...
if [ "$skip_verify" -eq 0 ]; then
  ...
  if command_exists cosign; then
    ...
    if ! cosign verify-blob \
      ...
      --signature "$sig_path" \
      --certificate-identity-regexp '^https://github.com/cmdrvl/fingerprint/.github/workflows/release\.yml@refs/(heads/main|tags/.*)$' \
...
binary_path="$extract_dir/fingerprint"
if [ ! -f "$binary_path" ]; then
  binary_path=$(find "$extract_dir" -type f -name fingerprint | head -n 1 || true)
fi

if [ -z "$binary_path" ] || [ ! -f "$binary_path" ]; then
  fail "fingerprint binary not found in archive"
fi
...
if (cd "$install_dir" && ln -sf "fingerprint@$version" "fingerprint" 2>/dev/null); then
  :
else
...
info "Rollback: ln -sf \"fingerprint@$version\" \"$active_binary\""
