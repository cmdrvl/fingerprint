...
| `region` | `anchor_selector`, `stop_selector`, `continue_past` | HTML selector-delimited bounds and contained table/page indices only | `{ "start_line": 30, "end_line": 180, "table_indices": [0, 1], "page_span": [5, 6] }` |
...
- `region` is HTML-only. Malformed `anchor_selector` or `stop_selector` values are definition errors; `continue_past` regexes suppress broad stop candidates such as repeated `(continued)` running headers. Region output must not include copied document text.
